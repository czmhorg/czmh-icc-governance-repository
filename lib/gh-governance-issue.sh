...
_gh-governance-issue-parse-step-repo-sync() {
  # Celý parse job workflow repo-sync (defs/defs-governance-repo.md,
  # repo-sync issue): naparsuje event ($GITHUB_EVENT_PATH) – tělo je
  ...
  # jen konverguje CODEOWNERS a webhook rep projektu k conf.d. Typy odmítnutí:
  ...
  # Použití: _gh-governance-issue-parse-step-repo-sync
  local _out="${GITHUB_OUTPUT:-/dev/stdout}" _event_path="${GITHUB_EVENT_PATH:?}"
  ...
  echo "Issue #$_number přijato: repo-sync projektu $_key (CODEOWNERS a webhook)."
}
...
_gh-governance-rename-body-parse() {
  ...
  # projektu, docs/implementovano/navrh/gh-rename.md): řádky project_key=, repo_name= a
...
_gh-governance-issue-reject-message() {
  ...
  case "$1" in
    ...
    unexpected_line)     echo "Tělo issue obsahuje neočekávaný řádek – povoleny jsou pouze řádky klíčů daného typu issue (project_key= a repo_name=; u move-repo navíc new_project_key=, u rename-repo new_repo_name=, u obou volitelný redirect=; u track-delete repo_path= a delete_issue=; u repo-sync jen project_key=)." ;;
    duplicate_key)       echo "Tělo issue obsahuje duplicitní klíč." ;;
    missing_key)         echo "V těle issue chybí povinný klíč (project_key, repo_name; u move-repo i new_project_key, u rename-repo i new_repo_name; u repo-sync jen project_key)." ;;
