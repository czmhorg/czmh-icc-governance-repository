...
# Bezpečný parser INI konfigurace conf.d/ (projects/, business-services/, profiles/).
# Načítá soubory klíč=hodnota do asociativního pole _GH_CONF bez sourcování.
# Formát a pravidla: docs/navrh/governance/zadani-konverze-confd.md.
...
_GH_CONF_PROJECT_FIELDS="business_service rulesets repository_teams repository_creators repository_archivers"
# Pole profilu mimo branch protection API: cílení rulesetu (vzor větví pro
# conditions.ref_name.include; viz docs/navrh/governance/prechod-rulesets.md).
...
_gh-conf-validate-bs() {
  # Zvaliduje business-services/<MHN>.conf: povinné klíče, neznámé klíče, login.
  # Použití: _gh-conf-validate-bs <MHN> <errors_ref>
  local _mhn="$1" _rel="business-services/$1.conf" _key _field _login
  ...
  for _key in "${!_GH_CONF[@]}"; do
    ...
    _field="${_key##*/}"
    [[ " display_name department jenkins_user " == *" $_field "* ]] || \
      _gh-conf-err "$2" "$_rel" "neznámý klíč '$_field'"
  done
  if [[ -v _GH_CONF["business-services/$_mhn/jenkins_user"] ]]; then
    _login="${_GH_CONF[business-services/$_mhn/jenkins_user]}"
...
_gh-conf-validate-project() {
  ...
  # Použití: _gh-conf-validate-project <key> <errors_ref> <profiles_seen_ref> <bs_seen_ref>
  local _pk="$1" _rel="projects/$1.conf" _field _key _value _bs _profile _item _rest
  local -A _rs_seen=()
  declare -n _vp_profiles="$3" _vp_bs="$4"
  ...
  _bs="${_GH_CONF[projects/$_pk/business_service]:-}"
  [[ -z "$_bs" || -v _vp_bs["$_bs"] ]] || \
    _gh-conf-err "$2" "$_rel" "business_service '$_bs' neodkazuje na existující business-services/$_bs.conf"
  ...
  if [[ -v _GH_CONF["projects/$_pk/rulesets"] ]] && \
    ...
    while [[ -n "$_rest" ]]; do
      ...
      _rs_seen["$_profile"]=1
      if [[ "$_item" == *'|jenkins' && -n "$_bs" && -v _vp_bs["$_bs"] && \
            ! -v _GH_CONF["business-services/$_bs/jenkins_user"] ]]; then
        _gh-conf-err "$2" "$_rel" "položka '$_item' v klíči 'rulesets' má atribut 'jenkins', ale business service '$_bs' nemá klíč 'jenkins_user'"
      fi
    done
  fi
}
...
_gh-conf-load() {
  ...
  local _root="$1" _name
  local -a _errors=()
  local -A _profiles_seen=() _bs_seen=() _projects_seen=()
  _GH_CONF=()
  ...
  for _name in "${_GH_CONF_PROJECT_KEYS[@]}"; do
    _gh-conf-validate-project "$_name" _errors _profiles_seen _bs_seen
  done
