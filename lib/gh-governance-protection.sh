...
# Bypass bota je jeho bypass tým (defs/defs.md: bypass tym; Team/always –
# actor typu User GHES 3.21 neuplatňuje).
# Závislosti: žádné (hodnoty bypassu a ID bypass týmu bota dodá volající).
...
_gh-governance-ruleset-payload() {
  ...
  # „povinný PR" jen přidala pravidla: bypass tým bota (Team, always – zápis
  # state/; ID týmu z _gh-bypass-team-ensure v gov-init) a podle
  # <admin_bypass> volitelně admini repa (RepositoryRole 5, pull_request).
  # required_linear_history se záměrně nepoužívá – gov repo povoluje jen
  # merge commit a lineární historie merge commity zakazuje
  # (docs/github/rulesets-required-linear-history-vs-merge-commit.md).
  # Použití: _gh-governance-ruleset-payload <bot_team_id> <admin_bypass: none|pull_request>
  local _bot_id="$1" _admin_bypass="$2" _bypass_actors
  if [[ ! "$_bot_id" =~ ^[0-9]+$ ]]; then
    echo "Chyba: Ruleset '$_GH_GOVERNANCE_RULESET_NAME' vyžaduje číselné actor_id bypass týmu bota (je '${_bot_id:-<prázdné>}')." >&2
    return 1
  fi
  _bypass_actors='{ "actor_id": '"$_bot_id"', "actor_type": "Team", "bypass_mode": "always" }'
